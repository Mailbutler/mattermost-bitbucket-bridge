    ...
    if event_key.startswith('pullrequest:'):
        if event_key.startswith('pullrequest:comment_'):
            ...
            if any(ext in comment_text for ext in bitbucket_ignore_comments):
                raise ValueError('Ignoring comment!')
        ...
        text = get_event_action_text(event_key).format(
            "[" + actor_name + "](" + actor_url + ")", "[#" + pr_id + "](" + pr_url + ")")
        ...
        for participant in data["pullrequest"]["participants"]:
            ...
            reviewers.append("{}[{}]({})".format(approved_icon, participant["user"]
                                                 ["display_name"], participant["user"]["links"]["html"]["href"]))
        if len(reviewers) > 0:
        ...
        text = "{}[{}]({}) {}".format(state_icon, data["commit_status"]
                                      ["name"], data["commit_status"]["url"], state_action)
...
def hooks(hook_path):
    ...
    if event == "diagnostics:ping":
        ...
        send_simple_webhook(
            hook_path, "diagnostics:ping", "Bitbucket is testing the connection to Mattermost: " + request_id, alert_color)
    else:
        if len(request.get_json()) > 0:
            data = request.get_json()
            if len(bitbucket_url) > 0:
                process_payload_server(hook_path, data)
            else:
                process_payload_cloud(hook_path, data, event)
    return ""
